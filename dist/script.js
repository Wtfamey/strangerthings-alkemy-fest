...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Safety check if text element exists
  if (text) {
    text.className += " intro-text--show";
  } else {
    // If specific Modernizr text blocks don't exist, bind anyway to ensure functionality
    bind = true;
  }
...
function startAnimation() {
  ...
  if (viewport) viewport.className += " viewport--show";
  if (letterbox) letterbox.className += " letterbox--show";

  // Set up credits to show every interval
  // Safety check: only run loop if credits exist
