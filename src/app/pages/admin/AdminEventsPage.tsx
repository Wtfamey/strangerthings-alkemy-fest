...
export function AdminEventsPage() {
    ...
    const fetchEventTypes = async () => {
        try {
            console.log("Admin: Fetching event types from BYPASS API...");
            const response = await fetch('/api/bypass.php?' + Date.now());
            const data = await response.json();
            console.log("Admin: Bypass API response:", data);
            if (data.success && data.data) {
                setEventTypes(data.data);
                console.log("Admin: Event types loaded:", data.data.length);
            } else {
                console.error("Admin: Failed to load event types:", data);
            }
        } catch (error) {
            console.error('Admin: Failed to fetch event types:', error);
        }
    };

    const fetchEvents = async () => {
        try {
            const response = await fetch('/api/events/read_with_registration.php');
            const data = await response.json();
            if (data.records) {
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div>
                                    <label className="block text-gray-300 text-sm font-medium mb-2">
                                        Event Type 
                                        {eventTypes.length > 0 && (
                                            <span className="text-green-400 text-xs ml-2">({eventTypes.length} available)</span>
                                        )}
                                    </label>
                                    <select 
                                        name="event_type_id" 
                                        ...
                                        className="w-full bg-gray-800 border border-gray-700 rounded-lg p-3 text-white text-base focus:border-red-500 focus:outline-none transition-colors appearance-none cursor-pointer" 
                                        required
                                    >
                                    ...
                                    {eventTypes.length === 0 && (
                                        <p className="text-red-400 text-xs mt-1">No event types available. Please check database.</p>
                                    )}
                                </div>
